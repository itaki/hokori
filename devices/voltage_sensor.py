...
class Voltage_Sensor:
    def __init__(self, volt_config, ads, status_callback):
        ...
        try:
            ...
            logger.debug(f"      🚥 ⚡︎ Current voltage reading is {reading}")
    ...
    def get_reading(self):
        if self.board_exists and not self.error_raised:
            try:
                ...
                logger.error(f"💢 ⚡︎ Attribute error while reading voltage: {e}")
                self.error_raised = True
                return 0
            except OSError as e:
                logger.error(f"💢 ⚡︎ I2C communication error: {e}")
                return 0
            except Exception as e:
                logger.error(f"💢 ⚡︎ Unexpected error while reading voltage: {e}")
                return 0
        else:
            return 0
    ...
    def in_good_range(self):
        if self.sensor_detection_threshold < self.reading < self.error_threshold:
            ...
            if self.sensor_detection_threshold > self.reading:
                logger.warning(f"🌟 ⚡︎ No sensor on pin {self.pin_number}")
            if self.reading > self.error_threshold:
                logger.warning(f"🌟 ⚡︎ Problem with sensor on pin {self.pin_number}")
    ...
    def set_trigger_voltage(self):
        ...
        if self.in_good_range():
            self.trigger = reading * self.multiplier  # Use multiplier from config
            logger.debug(f"      🚥 ⚡︎ Setting trigger point on pin {self.pin_number} to {self.trigger}")
